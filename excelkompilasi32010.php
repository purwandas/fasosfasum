...
$query = "SELECT * FROM  akunjournal aj  inner join bast b on aj.nobast=b.nobast inner join detaildokacuan d on b.nodokacuan=d.nodokacuan inner join dataaset s on aj.idaset=s.idaset inner join peruntukanhistory p on aj.idperuntukan=p.idperuntukan inner join lokasidokumen l on aj.nobast=l.nobastlokasi   where aj.idperuntukan NOT IN (SELECT idperuntukan FROM akun)  
...
while ($data = mysql_fetch_array($hasil))
{
   ...
   xlsWriteLabel($noBarisCell,10,$data['deskripsi']);
    ...
    // menampilkan data nama mahasiswa
   xlsWriteLabel($noBarisCell,11,$data['hkategoriaset']);

  // menampilkan data nama mahasiswa
   xlsWriteNumber($noBarisCell,12,$data['hvolume']);

  // menampilkan data nama mahasiswa
   xlsWriteLabel($noBarisCell,13,$data['hsatuan']);

   // menampilkan data nilai
   xlsWriteNumber($noBarisCell,14,$data['hnilaimix']);

   // menampilkan data nilai
   xlsWriteLabel($noBarisCell,15,$data['hketakun']);
