...
xlsWriteLabel(0,14,"NILAI NJOP"); 

// mengisi pada cell A5 (baris ke-0, kolom ke-6)
xlsWriteLabel(0,15,"NILAI BAST");  

// mengisi pada cell A5 (baris ke-0, kolom ke-6)
xlsWriteLabel(0,16,"NILAI MIX");  
...
$query = "SELECT * FROM akun a inner join bast b on a.nobast=b.nobast inner join detaildokacuan d on b.nodokacuan=d.nodokacuan inner join dataaset s on a.idaset=s.idaset inner join peruntukan p on a.idperuntukan=p.idperuntukan inner join lokasidokumen l on a.nobast=l.nobastlokasi  order by a.nobast";
...
while ($data = mysql_fetch_array($hasil))
{
  ...
  // menampilkan data nama mahasiswa
   ...
   xlsWriteNumber($noBarisCell,14,$data['jumnjop']);

   // menampilkan data nilai
   xlsWriteNumber($noBarisCell,15,$data['nilaibast']);

  // menampilkan data nilai
   xlsWriteNumber($noBarisCell,16,$data['nilaimix']);
